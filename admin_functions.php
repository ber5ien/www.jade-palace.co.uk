...
function home_summary() 
{
	...
	$result = $mysqli->query("select * from users where region='Roche'");
	echo "<h1>Roche's acccounts: ".$result->num_rows."</h1>";
	$result = $mysqli->query("select * from users where region='St.Austell'");
	echo "<h1>St.Austell's accounts: ".$result->num_rows."</h1>";
	$result = $mysqli->query("select * from users where region='Truro'");
	echo "<h1>Truro's accounts: ".$result->num_rows."</h1></center>";
}
...
function delivery_update()
{
	...
	while( $data=$result->fetch_object() )
	{
		if(isset($_GET['action']) && $_GET['action']=='amend' && $_GET['id'] == $data->id_delivery)
		{
			...
			if ( $data->takeaway == "staustell")
			{
			echo ("<td><select name=takeaway><option SELECTED>staustell</option><option>truro</option><option>roche</option></select></td>");
			}elseif( $data->takeaway == "truro"){
			echo ("<td><select name=takeaway><option>staustell</option><option SELECTED>truro</option><option>roche</option></select></td>");
			}else{
			echo ("<td><select name=takeaway><option>staustell</option><option>truro</option><option SELECTED>roche</option></select></td>");
			}
